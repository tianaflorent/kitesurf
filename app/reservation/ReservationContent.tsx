  ...
  return (
    <main className="bg-gray-50 min-h-screen flex flex-col items-center justify-start py-16 px-6">
      <div className="max-w-3xl w-full bg-white rounded-2xl shadow-lg p-8 md:p-12 mb-20">
        <h1 className="text-3xl font-bold text-cyan-700 mb-8 text-center">{t.title}</h1>
        ...
        <form className="grid grid-cols-1 gap-6" onSubmit={(e) => { e.preventDefault(); handleSubmit(); }}>
          ...
          <div>
            ...
            <input type="number" min={1} required value={nombre} onChange={(e) => setNombre(Number(e.target.value))}
              className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cyan-600" />
          </div>

          <button type="submit" className="w-full bg-cyan-600 text-white py-3 rounded-lg hover:bg-cyan-700 transition font-semibold">
            {t.send}
          </button>
        </form>
      </div>
