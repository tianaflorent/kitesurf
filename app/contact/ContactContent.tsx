    ...
    <main className="pb-10 bg-linear-to-br from-orange-50 via-white to-blue-50">
      ...
      <section className="relative h-75 mt-7 rounded-2xl overflow-hidden border border-gray-300 shadow-md">
        ...
        <div className="relative z-10 h-full flex flex-col justify-center px-6 text-white">
          ...
          <p className="text-sm leading-relaxed max-w-sm">{t.heroText}</p>
        </div>
      </section>

      {/* FORMULAIRE */}
      <section className="mx-4 mt-6 bg-white rounded-3xl p-5 border border-gray-300 shadow-md">
        <h2 className="text-lg font-semibold text-center mb-4">{t.formTitle}</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <label className="block font-serif font-semibold">{t.name}</label>
          ...
          <label className="block font-serif font-semibold">{t.email}</label>
          <input type="email" placeholder={t.emailPlaceholder} value={email} onChange={(e) => setEmail(e.target.value)} required
            className="w-full border border-gray-300 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-blue-600" />

          <label className="block font-serif font-semibold">{t.phone}</label>
          <input type="tel" placeholder={t.phonePlaceholder} value={phone} onChange={(e) => setPhone(e.target.value)} required
            className="w-full border border-gray-300 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-blue-600" />

          <label className="block font-serif font-semibold">{t.message}</label>
          <textarea placeholder={t.messagePlaceholder} rows={4} value={message} onChange={(e) => setMessage(e.target.value)} required
            className="w-full border border-gray-300 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-blue-600" />

          <button type="submit" disabled={loading}
            className={`w-full cursor-pointer bg-blue-600 hover:bg-blue-700 transition text-white font-semibold py-3 rounded-xl flex items-center justify-center gap-2 shadow-md ${loading ? "opacity-70 scale-95" : "opacity-100"}`}
          >
            {loading ? <span className="animate-pulse">{t.preparing}</span> : <>{t.send} <Send size={18} /></>}
          </button>
        </form>
      </section>
      ...
      <section className="mx-4 mt-8 space-y-4">
        <InfoCard icon={<Phone />} title={t.phoneTitle} subtitle="+261 37 71 473 00" note={t.phoneNote} />
        <InfoCard icon={<Mail />} title={t.emailTitle} subtitle="purewindmadakiteschool@gmail.com" note={t.emailNote} />
        <InfoCard icon={<MapPin />} title={t.addressTitle} subtitle="Baie de Sakalava, Antsiranana (Diego Suarez), Madagascar" note={t.addressNote} />
        <InfoCard icon={<Clock />} title={t.hoursTitle} subtitle={t.hoursWeek} note={t.hoursWeekend} />
      </section>
...
function InfoCard({ icon, title, subtitle, note }: { icon: React.ReactNode; title: string; subtitle: string; note: string }) {
  return (
    <div className="bg-white rounded-2xl p-4 flex gap-4 cursor-pointer items-start border border-gray-300 shadow-sm">
      <div className="w-12 h-12 rounded-full text-white bg-blue-600 flex items-center justify-center">{icon}</div>
      <div>
