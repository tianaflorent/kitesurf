...
export const NavBody = ({ children, className, visible }: NavBodyProps) => {
  return (
    <motion.div
      ...
      className={cn(
        "relative z-[60] mx-auto hidden w-full flex-row items-center justify-between self-start px-6 lg:px-12 py-4 lg:flex transition-colors duration-500",
        visible ? "bg-background/80 text-foreground" : "bg-transparent text-white",
        className,
      )}
    >
      {children}
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        visible ? "bg-background/90 text-foreground" : "bg-transparent text-white",
        className,
      )}
    >
      {children}
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          exit={{ opacity: 0, y: -10 }}
          className={cn(
            "absolute inset-x-0 top-16 z-50 flex w-full flex-col items-start justify-start gap-6 rounded-b-2xl bg-background px-6 py-8 shadow-xl border-b border-border text-foreground",
            className,
          )}
        >
          {children}
...
export const MobileNavToggle = ({
  ...
  return isOpen ? (
    <IconX className="w-6 h-6" onClick={onClick} />
  ) : (
    <IconMenu2 className="w-6 h-6" onClick={onClick} />
  );
};
