    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          exit={{ opacity: 0, y: -10 }}
          className={cn(
            "absolute inset-x-0 top-16 z-50 flex w-full flex-col items-start justify-start gap-6 rounded-none bg-background px-6 py-8 shadow-xl border-b border-border text-foreground",
            className,
          )}
        >
          {children}
